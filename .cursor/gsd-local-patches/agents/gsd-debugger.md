...
Installer says:   hooksDest = path.join(targetDir, 'hooks')
                  targetDir = .cursor/get-shit-done
                  → writes to .cursor/get-shit-done/hooks/
...
```bash
INIT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" state load)
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: resolve debug {slug}" --files .planning/debug/resolved/{slug}.md
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: update debug knowledge base with {slug}" --files .planning/debug/knowledge-base.md
```
