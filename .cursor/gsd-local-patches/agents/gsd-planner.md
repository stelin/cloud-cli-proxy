...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "fix($PHASE): revise plans based on checker feedback" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" history-digest
```
...
```bash
VALID=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter validate "$PLAN_PATH" --schema plan)
```
...
```bash
STRUCTURE=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" verify plan-structure "$PLAN_PATH")
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs($PHASE): create phase plan" --files .planning/phases/$PHASE-*/$PHASE-*-PLAN.md .planning/ROADMAP.md
```
</step>
