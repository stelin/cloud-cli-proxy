...
```bash
INIT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" init milestone-op)
...
```bash
ROADMAP=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap analyze)
```
...
```bash
DETAIL=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM})
```
...
```bash
PHASE_STATE=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" init phase-op ${PHASE_NUM})
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): auto-generated context (discuss skipped)" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
...
```bash
PHASE_SECTION=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase ${PHASE_NUM} 2>/dev/null)
...
```bash
UI_REVIEW_CFG=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" config-get workflow.ui_review 2>/dev/null || echo "true")
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(${PADDED_PHASE}): smart discuss context" --files "${phase_dir}/${padded_phase}-CONTEXT.md"
```
