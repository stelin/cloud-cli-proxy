...
```bash
INIT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" init new-project)
if [[ "$INIT" == @file:* ]]; then INIT=$(cat "${INIT#@file:}"); fi
AGENT_SKILLS_RESEARCHER=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-project-researcher 2>/dev/null)
AGENT_SKILLS_SYNTHESIZER=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-synthesizer 2>/dev/null)
AGENT_SKILLS_ROADMAPPER=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" agent-skills gsd-roadmapper 2>/dev/null)
```
...
```bash
mkdir -p .planning
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "chore: add project config" --files .planning/config.json
```
...
```bash
mkdir -p .planning
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: initialize project" --files .planning/PROJECT.md
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: define v1 requirements" --files .planning/REQUIREMENTS.md
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" generate-claude-md
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs: create roadmap ([N] phases)" --files .planning/ROADMAP.md .planning/STATE.md .planning/REQUIREMENTS.md .cursor/rules/
```
...
```bash
PHASE1_SECTION=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase 1 2>/dev/null)
