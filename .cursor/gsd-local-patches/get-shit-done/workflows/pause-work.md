...
```bash
timestamp=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" current-timestamp full --raw)
```
...
<step name="commit">
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "wip: [phase-name] paused at task [X]/[Y]" --files .planning/phases/*/.continue-here.md .planning/HANDOFF.json
```
</step>
