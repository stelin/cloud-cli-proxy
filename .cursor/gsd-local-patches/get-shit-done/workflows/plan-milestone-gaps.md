...
HIGHEST=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" phases list --pick directories[-1])
```
...
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "docs(roadmap): add gap closure phases {N}-{M}" --files .planning/ROADMAP.md .planning/REQUIREMENTS.md
```
