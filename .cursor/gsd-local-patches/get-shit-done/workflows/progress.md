...
```bash
INIT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" init progress)
...
- `STATE=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" state-snapshot)`
...
- Use `summary-extract` for efficient parsing:
  ```bash
  node ".cursor/get-shit-done/bin/gsd-tools.cjs" summary-extract <path> --fields one_liner
  ```
...
PROGRESS_BAR=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" progress bar --raw)
```
...
```bash
DEBT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" audit-uat --raw 2>/dev/null)
```
...
```bash
PHASE_SECTION=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "${CURRENT_PHASE}" 2>/dev/null)
...
```bash
NEXT_PHASE_SECTION=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" roadmap get-phase "$((Z+1))" 2>/dev/null)
