...
```bash
INIT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" init verify-work "${PHASE_ARG}")
...
```bash
CHECKPOINT=$(node ".cursor/get-shit-done/bin/gsd-tools.cjs" uat render-checkpoint --file "$uat_path" --raw)
...
Commit the UAT file:
```bash
node ".cursor/get-shit-done/bin/gsd-tools.cjs" commit "test({phase_num}): complete UAT - {passed} passed, {issues} issues" --files ".planning/phases/XX-name/{phase_num}-UAT.md"
```
