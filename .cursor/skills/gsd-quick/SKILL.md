...
- Check if SUMMARY.md exists; if so, read `status` from its frontmatter via:
  ```bash
  node "/Users/zaneliu/Projects/open-source/cloud-cli-proxy/.cursor/get-shit-done/bin/gsd-tools.cjs" frontmatter get .planning/quick/{dir}/SUMMARY.md --field status 2>/dev/null
  ```
...
5. Load context via:
   ```bash
   node "/Users/zaneliu/Projects/open-source/cloud-cli-proxy/.cursor/get-shit-done/bin/gsd-tools.cjs" init quick
   ```
...
Execute the quick workflow from @/Users/zaneliu/Projects/open-source/cloud-cli-proxy/.cursor/get-shit-done/workflows/quick.md end-to-end.
