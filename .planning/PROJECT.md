...
**Status:** ◆ Planning — requirements and roadmap in progress

## Shipped Milestone: v4.0 sing-box 同容器化 (Shipped 2026-05-27, tag v4.0)

**Goal:** 把 sing-box 从独立 `cloudproxy-gw-*` 容器搬进用户容器内部，消灭 "user 容器 default 路由不指向 gw" 这一整类回归泄漏；单容器架构带来 breaking change，故升 major v4.0。

**Target features:**

- **F1** managed-user 镜像内置 sing-box，entrypoint 作 PID 1 + setuid 降权
- **F2** 控制面去掉 gw 容器路径与 `cloudproxy-net-*` 自定义 bridge，`container_proxy_provider` 简化为单容器
...
**Active:** Awaiting next milestone (v4.0 shipped 2026-05-27)
...
---
*Last updated: 2026-05-28 — v4.0 sing-box 同容器化 milestone shipped & archived (4 phases: 53-56, 13 plans, 28/28 REQ, audit passed, tech debt cleaned). Awaiting next milestone.*
