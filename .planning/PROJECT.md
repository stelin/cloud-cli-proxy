...
## Next Milestone Goals (v3.1 增量优化)

> v3.0 核心已 ship，v3.1 聚焦在 v3.0 暴露的真实需求与 backlog。

候选方向（待 `/gsd-new-milestone` 时收敛）：
...
- 3 项真机签字 ship 闸门（M5 APFS / BASE-03 2min / C6 Ubuntu 25.04）
...
### Active (v3.1 候选 — 待 /gsd-new-milestone 收敛)

- [ ] 容器预热与空闲回收策略（避免每次冷启动 5-10s 等待，涉及控制面资源调度）
- [ ] 性能 metrics 实时上报到 admin 后台（首连耗时、mount 模式、抖动事件分布）
- [ ] admin 后台 host 详情页展示 mount 模式 / session 数 / persistent volume 列表
- [ ] Mutagen 首轮全量同步针对 mono-repo 的优化策略（lazy probe / progressive scan）
- [ ] v3.0 deferred 真机签字闸门（M5 APFS / BASE-03 2min / C6 Ubuntu 25.04）— ship 前补完
- [ ] v3.0 tech debt 批次清理（≈14 项 Phase 32/33 WR/HR/MR + spec/code 数字漂移）
...
---
*Last updated: 2026-04-23 after v3.0 milestone — 33/34 REQ satisfied，BASE-02 + 3 项真机签字 deferred-to-ship；ready for v3.1 planning*
