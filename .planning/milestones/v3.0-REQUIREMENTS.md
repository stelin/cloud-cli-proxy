...
- [x] **REQ-F4-B**：用户可通过 `cloud-claude sessions ls` / `cloud-claude sessions attach <name>` 管理多个并行会话
- [x] **REQ-F4-C**：当容器内 tmux 不可用时 cloud-claude 不得阻塞启动，但必须在 banner 明确提示 `[!] 容器内 tmux 不可用，会话恢复已禁用`
...
- [x] **REQ-F7-A**：`~/.claude/` 与 `~/.cache/claude` 通过独立 Docker named volume 持久化；命名粒度 = 单个 `claude_account`（建议 `claude-state-{claude_account_id}`，带 label `com.cloud-cli-proxy.account_id`）
...
| REQ-F4-B | Phase 32 | Complete | `cloud-claude sessions ls/attach` |
