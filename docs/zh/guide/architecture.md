...
- **网络配置** — 创建网络命名空间、配置 WireGuard 接口或 sing-box tun
...
- 主机记录、短 ID、SSH 密码和状态
- 出口 IP 配置（WireGuard 参数和代理配置 JSONB）
- 主机与出口 IP 绑定关系
- 异步任务记录
- 审计事件（13 种事件类型）
...
├── wg0 (WireGuard 接口)
│   └── 路由：0.0.0.0/0 → wg0
└── nftables：默认拒绝，仅允许 WireGuard 隧道流量
```

使用 birthplace-namespace 模式：WireGuard 接口在宿主机 namespace 创建后，通过 `ip link set wg0 netns ...` 移入容器 namespace。密钥配置在容器 namespace 内完成，不经过宿主机网络栈。
...
- WireGuard 私钥由 host-agent 自动生成，存储在 `DATA_DIR` 中
- 容器 SSH 密码独立于用户登录密码
...
│   ├── network/                # WireGuard / nftables / sing-box 网络配置
