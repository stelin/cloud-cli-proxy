...
// UserPasswordStore 用户自助修改登录密码与 SSH 密码
...
type changeSSHPasswordRequest struct {
	OldPassword   string `json:"old_password"`
	NewSSHPassword string `json:"new_ssh_password"`
}

func (h *UserPasswordHandler) ChangeSSHPassword() nethttp.Handler {
	return nethttp.HandlerFunc(func(w nethttp.ResponseWriter, r *nethttp.Request) {
		userID := UserIDFromContext(r.Context())
		if userID == "" {
			writeJSON(w, nethttp.StatusUnauthorized, map[string]string{"error": "unauthorized"})
		...
		req.OldPassword = strings.TrimSpace(req.OldPassword)
		req.NewSSHPassword = strings.TrimSpace(req.NewSSHPassword)
		if req.OldPassword == "" || req.NewSSHPassword == "" {
			writeJSON(w, nethttp.StatusBadRequest, map[string]string{"error": "old_password and new_ssh_password are required"})
			return
		}
		if len(req.NewSSHPassword) < 6 || len(req.NewSSHPassword) > 128 {
			writeJSON(w, nethttp.StatusBadRequest, map[string]string{"error": "new ssh password must be 6-128 characters"})
			return
		}
		...
		if err != nil {
			...
			h.logger.Error("change ssh password: get user failed", "user_id", userID, "error", err)
			writeJSON(w, nethttp.StatusInternalServerError, map[string]string{"error": "internal error"})
			return
		}

		if err := bcrypt.CompareHashAndPassword([]byte(user.PasswordHash), []byte(req.OldPassword)); err != nil {
			writeJSON(w, nethttp.StatusUnauthorized, map[string]string{"error": "invalid login password"})
			return
		}

		if err := h.store.UpdateUserEntryPassword(r.Context(), userID, req.NewSSHPassword); err != nil {
			if errors.Is(err, pgx.ErrNoRows) {
				writeJSON(w, nethttp.StatusNotFound, map[string]string{"error": "user not found"})
				return
			}
			h.logger.Error("change ssh password failed", "user_id", userID, "error", err)
			writeJSON(w, nethttp.StatusInternalServerError, map[string]string{"error": "change ssh password failed"})
			return
		}
